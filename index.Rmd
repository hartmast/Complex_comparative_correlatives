...
ggplot(dtababy, aes(x = year, y = text_type, size = freq, col = author)) +
  ...
  geom_boxplot(aes(group = text_type), notch = T, alpha = .5, col = "grey") +
  scale_color_viridis_d() +
  theme_bw() + ylab("Text type") + xlab("Year") +
...
# ggsave("fig/dtababy_distribution_EN.png", width = 9, height = 6.5)
# ggsave("fig/dtababy_distribution_with_boxplot_EN.png", width = 9, height = 6.5)
...
dtababy %>% mutate(
  ...
  guides(color = "none")
# ggsave("fig/kks_rel_freq_annotated01_EN.png", width = 12, height = 5.5)  
...
d %>% select(file_century, file_genre) %>% table %>% as_tibble() %>% pivot_wider(names_from = "file_genre", values_from = "n") %>% DT::datatable()
...
# ggsave("fig/structure_EN.png", width = 10, height = 6)
...
cosenty <- function(i) {
  p <- d %>% mutate(Order = ifelse(grepl("^A", d$structure), "Apodosis > Protasis", "Protasis > Apodosis")) %>%
    ...
    qbarplot(file_century, Order, KK_Nebensatz, wrap100=T) + xlab("Century") + ylab("Relative Frequency") +
...
# ggsave("fig/Order_main_subordinate_EN_portrait.png", width = 10, height= 15  )
...
# ggsave("fig/connectors_EN.png", width = 10, height = 5.5)
...
# ggsave("fig/structure_by_connectors_EN_portrait.png", width = 7, height = 13)
...
# ggsave("fig/Verb_position_EN.png", width = 10, height = 6)
...
# ggsave("fig/verbstellung_same_different_connectors_EN_portrait.png", width = 7, height = 13)
...
# EN
d$Elliptical_Antecedent_in_Consequent <- ifelse(d$gemeinsamer_Bezug=="nein", "elliptical A in C", "no elliptical A in C")
d$Elliptical_Antecedent_in_Consequent <- factor(d$Elliptical_Antecedent_in_Consequent, levels = c("elliptical A in C", "No elliptical A in C"))
...
mca_input <- select(d, file_genre, Century, Connector, Clause_Order, 
                    Elliptical_Antecedent_in_Consequent, Sentence_Type, Verb_position,
...
fviz_mca_var(mca_results, choice = "var.cat", col.quali.sup = "red",
  ...
  theme(legend.title = element_text(size = 18, face = "bold")) +
  theme(text = element_text(size = 18))
ggsave("fig/mca02.png", height = 13, width = 13, bg = "white")
...
# ggsave("fig/authors_salience_en_transp.png", width = 15, height = 10, bg = "transparent")
# ggsave("fig/authors_salience_en.png", width = 15, height = 10, bg = "transparent")


# salience overall
set.seed(2000)
d_salience %>% mutate(Author = ifelse(mean_salience>=1, gsub(".* ", "", author), "")) %>%
  ggplot(aes(x = year, y = mean_salience)) + facet_wrap(~text_type) +
  geom_point(col="grey30", fill = "transparent") + 
  geom_smooth() + theme_bw() + geom_text_repel(aes(label = Author), vjust = -0.8, cex = 2.5, max.overlaps = 18) +
  ggtitle("Salience index") +
  theme(plot.title = element_text(face = "bold", hjust = 0.5),
        panel.background = element_rect(fill='transparent'), #transparent panel bg
        plot.background = element_rect(fill='transparent', color=NA)) +
  xlab("Year") + ylab("Mean salience") + theme(rect = element_rect(fill = "transparent"))
 ggsave("fig/salience_en.png", width = 15, height = 6, bg = "white")

```

## Additional metadata: Age of authors

The birth year of authors was automatically extracted from the German National Library using the following function (based on a script kindly provided by Andreas Blombach):
...
get_author_details <- function(author_id) {
  tryCatch(
    {json_data <- fromJSON(str_c("https://hub.culturegraph.org/entityfacts/", author_id), flatten = TRUE)
    
    if (!is_null(json_data$placeOfBirth$`@id`)) {
      place_of_birth_coords <- json_data$placeOfBirth$`@id` |>
        str_extract("(https://d-nb.info/gnd/)(.+)$", group = 2) |>
        get_coordinates() |>
        as.double()
    } else {
      place_of_birth_coords <- NA
    }
    
    tibble_data <- tibble(
      author_id = author_id,
      name = json_data$preferredName,
      surname = json_data$surname,
      forename = json_data$forename,
      gender = json_data$gender$label,
      date_of_birth = json_data$dateOfBirth,
      date_of_death = json_data$dateOfDeath,
      place_of_birth = json_data$placeOfBirth$preferredName,
      longitude = place_of_birth_coords[1],
      latitude = place_of_birth_coords[2]
    )
...
get_coordinates <- function(place_id) {
  json_data <- fromJSON(str_c("https://hub.culturegraph.org/entityfacts/", place_id), flatten = TRUE)
  coords <- json_data$location$geometry$coordinates
  return(coords)
}
...
# List of DTAbaby IDs
dtababy <- read_delim("dtababy_textnames.txt", delim="\t", quote="",
                      col_names = c("Length", "ID", "Title", "Genre"))

# get author IDs ----------------------------------------------------------
f <- list.files("dta2021_convert/files/full", full.names = T) # downloadable data from deutschestextarchiv.de

dtababy$author_ID_link <- NA

for(i in 1:nrow(dtababy)) {
  if(any(grepl(dtababy$ID[i], f))) {
    cur <- readLines(f[grep(dtababy$ID[i], f)])
    
    start <- grep("<cmdp:author>", cur)[1]
    end <- grep("</cmdp:author>", cur)[1]
    
    if(!is.na(start) & !is.na(end)) {
      cur <- cur[start:end]
      
      if(any(grepl("d-nb.info", cur))) {
        cur <- grep("d-nb.info", cur, value=T)
        dtababy$author_ID_link[i] <- gsub("<.*", "", gsub(".*(?=http://d-nb)", "", cur, perl = T))
      }
...
dtababy$Author_ID <- gsub(".*/", "", dtababy$author_ID_link)

# get unique author IDs, map author data
ids <- unique(na.omit(dtababy$Author_ID))
author_data <- map_dfr(ids, get_author_details)

# combine with df and export
dtababy <- left_join(dtababy, author_data, by = c("Author_ID" = "author_id"))
write_csv(dtababy, "dtababy_with_author_data.csv")
...
d_salience %>% filter(!is.na(Geburtsjahr)) %>%
  ...
  ggplot(aes(x = text_type, y = salience, col = century, fill = Geburtsjahr)) +
  geom_boxplot() + # facet_wrap(~text_type) +
  xlab("Text type") + ylab("Salience index") +
  guides(col = guide_legend(ncol = 2)) +
  theme_bw() +
  scale_color_manual(name = "Century", values = rep("black", 3)) +
  scale_fill_manual(name = "Year of birth", values = c(RColorBrewer::brewer.pal(4, "Purples"), RColorBrewer::brewer.pal(3, "Blues"), RColorBrewer::brewer.pal(5, "Oranges")[2:4], RColorBrewer::brewer.pal(5, "Greens")[1:3])) +
  theme(axis.text = element_text(size = 18)) +
  theme(axis.title = element_text(size = 18)) +
  theme(strip.text = element_text(size = 18)) +
  theme(legend.text = element_text(size = 18)) +
  ...
  #guides(fill = guide_legend(title = "Year of birth")) +
  guides(color = "none")
# ggsave("fig/Author_salience_index_Age_EN.png", width = 9, height = 6)
